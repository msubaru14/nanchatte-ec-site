...
- 調査はタスク目的に対して必要十分にする
- `docs/設計ドキュメント索引.md` は、読み込み量を減らすための入口資料として優先的に確認する
- README、`docs/api/API.md`、`docs/api/openapi.yaml`、長いドメイン資料、長いテストファイルは、まず `rg -n` / 見出し検索 / 周辺行確認で必要箇所を特定する
- 長いテストファイルは、まず `rg -n "func Test"` や対象メソッド名で関連テストを探し、必要なテストケース周辺を確認する
- `docs/api/openapi.yaml` は、path / operationId / schema名 / response名で検索し、該当箇所と周辺行を確認する
- 長いファイルの全文読みは、全体構造の把握が必要な場合、または検索・周辺行確認だけでは判断できない場合に限る
- 精度が必要な仕様・API契約・エラー形式は、該当箇所の周辺行を十分に確認する
...
Goファイル変更後は、原則として作業完了前またはPR作成前に以下を実行する。
...
実装途中や小さな縦割りコミット前は、変更対象 package のテストに絞ってよい。

例:

```powershell
cd backend
go test ./internal/review ./internal/router
```

ただし、共有処理・middleware・router・DB・複数domainに影響する変更、不具合調査、または関連 package のテストだけでは影響範囲を確認しきれない場合は、早めに `go test ./...` を実行する。
コミット前に `go test ./...` を未実施にする場合は、作業完了前またはPR作成前にまとめて実行する予定であることを報告する。
...
- 差分確認はまず `git diff --stat` / `git diff --cached --stat` で全体像を把握する
- 詳細diffは、変更の妥当性確認に必要なファイル・箇所に絞って確認する
- コミット前に `git status` と staged diff を確認する。staged diff もまず `git diff --cached --stat` を使い、必要な範囲だけ詳細diffを見る
- `git status` / `git diff --stat` / `git log` などの同じ確認は、作業開始・コミット前・PR作成前などの節目を基本とし、必要以上に繰り返さない
...
Commander から特別な指定がない限り、手動API確認では以下の検証用ユーザーを優先する。

- email: `codex@test.com`
- password: 平文で記録せず、必要なタイミングで Commander に確認する

## 報告ルール

作業中の報告は、作業開始、編集前、検証前、完了時などの節目を基本とする。
細かいコマンドごとの報告は、失敗・方針変更・追加確認が必要な場合に限る。
