...
- DB内部識別子
- Phase1の注文履歴APIでは、詳細取得用のAPI識別子として利用する
- UI表示・問い合わせ・ユーザー向け案内では利用しない
...
Phase1では、Cartに購入不可商品が1件でも含まれる場合は注文全体を失敗させる。

購入可能商品のみを部分的に注文することは扱わない。
購入不可商品の削除・数量調整は、注文確定前にCart API / frontend側で行う。
...
- 在庫減算しない
- cart_items は削除しない
- frontend側でCartを再取得し、削除・数量調整を促す
...
Phase1の注文履歴APIでは、以下のエンドポイントで `orders.id` をAPI識別子として利用する。

```txt
GET /api/orders/:id
```

ただし、画面表示・問い合わせ・ユーザー向け案内では `order_number` を優先する。
`orders.id` はAPI内部の対象指定に使う値であり、ユーザー向けの注文番号として扱わない。

他ユーザーの注文IDを指定した場合は、注文の存在有無を漏らさないため `NOT_FOUND` として扱う。
